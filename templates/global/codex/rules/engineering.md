...
Use isolated subagents for broad/noisy investigations when they reduce context pollution or provide independent reasoning. Do not use subagents for trivial work. Subagents consume their own tokens; unnecessary fan-out is wasteful.

Filter verbose shell output before reading it. Target relevant errors, timestamps, request IDs, or symbols.

For long-running work, persist durable state in project docs when available and start a fresh thread when switching to a substantially different task.
