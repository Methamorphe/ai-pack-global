...
usage(){ echo "Usage: ./uninstall-ai-global.sh [--cursor-only|--codex-only|--claude-only|--copilot-only|--dry-run]"; }
...
COPILOT_AGENTS=(researcher.agent.md architect.agent.md debugger.agent.md reviewer.agent.md ui-designer.agent.md visual-reviewer.agent.md)
...
if [[ $COPILOT -eq 1 ]]; then
  run rm -f "$COPILOT_HOME/instructions/engineering.instructions.md"
  for a in "${COPILOT_AGENTS[@]}"; do run rm -f "$COPILOT_HOME/agents/$a"; done
  for s in "${SKILLS[@]}"; do run rm -rf "$COPILOT_HOME/skills/$s"; done
fi

echo "Known AI Global Pack v3.2 files removed."
